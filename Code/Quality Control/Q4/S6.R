...
setwd("C:\\Users\\MANJU VISVANATHAN\\Desktop\\Stats Project\\subject06\\session1")
list.files()

Subject_Session1_Cutting = read.table("Subject06_Cutting1.csv",header = TRUE, sep=",",stringsAsFactors = FALSE)

Subject_Session1_Surturing = read.table("Subject06_Suturing1.csv",header = TRUE, sep=",",stringsAsFactors = FALSE)

Subject_Session1_Baseline = read.table("Subject06_Baseline1.csv",header = TRUE, sep=",",stringsAsFactors = FALSE)
...
plot(wZb, type="l", lty=1 ,xlim = c(0,max(Max_Time_B,Max_Time_C,Max_Time_S)), ylim = c(0,max(wZb,wZc,wZs)) ,xlab="Time in seconds", ylab= expression(paste("Perinasal Perspiration [",degree,"C"^2,"]")), main="Subject 6 : Session1 Stress Signals", col="black")
